        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            if (!string.IsNullOrEmpty(redisAddress))
            {
                services.AddStackExchangeRedisCache(options =>
                {
                    ...
                    {
                        // Handle full URI for production-like environments
                        var redisUri = new Uri(redisAddress);
                        var configurationOptions = new ConfigurationOptions
                        {
                            EndPoints = { { redisUri.Host, redisUri.Port } },
                            Ssl = redisUri.Scheme == "rediss",
                            AbortOnConnectFail = false // Recommended for robustness
                        };

                        if (!string.IsNullOrEmpty(redisUri.UserInfo))
                        {
                            var userInfo = redisUri.UserInfo.Split(new[] { ':' }, 2);
                            if (userInfo.Length > 0) configurationOptions.User = userInfo[0];
                            if (userInfo.Length > 1) configurationOptions.Password = userInfo[1];
                        }
                        options.ConfigurationOptions = configurationOptions;
                    }
                    else
                    {
                        // Handle simple "host:port" for development
